...
| LinsightSkillPolicy（`linsight_skill_policy` 表） | F056-cofco-hidden-office-skills | 租户范围 Skill 展示与运行时策略；本期仅拥有 `frontend_hidden`。隐藏项不进入业务前台列表/详情/选择接口，但启用时由任务模式服务端强制物化；停用优先。策略仅全局超级管理员可写，变更进入结构化审计；不改变 F035 对 Skill 元数据、bundle 和启停状态的归属 |
...
| F056-cofco-hidden-office-skills | F035（Skill 元数据、bundle、任务模式物化链路）、F011/F012/F013（租户隔离）、全局超级管理员与管理范围基线 | 中粮专版增量：新增租户策略表 `linsight_skill_policy` 与超级管理员管理字段；业务 API 对隐藏项按不存在处理，任务模式服务端合并用户选择与启用隐藏项；提供默认 dry-run 的 docx/pptx/xlsx 租户初始化脚本。不新增错误码或跨 Feature 不变量，不反向改变其他租户与标品默认值 |
...
| 2026-08-12 | 登记 F056 中粮专版预置 Skill 前端隐藏：新增租户策略对象 `LinsightSkillPolicy` / `linsight_skill_policy` 表；隐藏项仅全局超级管理员在管理后台可见，业务接口按不存在处理，任务模式服务端对启用隐藏项强制物化且停用优先；提供 docx/pptx/xlsx dry-run 初始化脚本；复用 11053/11054，不新增错误码或跨 Feature 不变量 | F056 / F035 |
