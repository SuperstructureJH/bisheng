...
| UserPreference（`user_preference` 表） | F042-personal-font-size | 当前账号的电脑端个人显示偏好；本期仅拥有 `font_scale_level`（内部 1–7 档）的读取与写入。该对象按全局用户账号归属，不随租户切换，不修改源文档、导出内容或其他用户的显示 |
...
| 2026-07-24 | 登记 F042 个人字体大小：新增账号级 `UserPreference` 领域对象与 `user_preference` 表，提供 1–7 档字号偏好读写；标品显示 7 档吸附滑杆，中粮版本映射 1/3/5 三档；不新增错误码、不修改源内容与导出内容、不新增跨 Feature 不变量 | F042 |
