...
| —（无新增） | F043-workbench-knowledge-space-grouping | 只调整工作台 `ChatKnowledge` 的知识空间下拉分组展示；复用既有 mine / joined / department 列表接口和选择状态，不新增领域对象、表、API、权限、错误码或写行为 |
...
| F043-workbench-knowledge-space-grouping | — | 纯前端展示调整；复用现有知识空间列表接口与知识空间页分类口径，不新增领域对象、API、错误码或跨 Feature 不变量 |
...
| 2026-07-27 | 登记 F043 工作台知识空间下拉分类：只在 `ChatKnowledge` 中按部门 / 我创建的 / 我加入的分组展示，搜索和选择交互保持现状；无领域对象、API、权限、错误码或不变量变化 | F043 |
