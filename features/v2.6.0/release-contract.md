...
| —（无新增） | F044-knowledge-space-quick-qa-selection | 知识空间列表勾选内容作为快速问答显式范围；扩展既有 folder chat 请求体并复用 F029 `KnowledgeFileVisibilityService`，不新增领域对象、表、API 路径、权限、错误码或写行为 |
...
| F044-knowledge-space-quick-qa-selection | F029 | 扩展直接知识空间 AI 问答的候选范围；文件夹递归展开、显式文件范围与标签取交集，最终结果继续遵守 INV-7 的双层 `view_file` 过滤；不新增领域对象、表、API 路径、错误码或不变量 |
...
| 2026-07-27 | 登记 F044 知识空间已选内容快速问答：列表勾选统一驱动输入框引用卡片与 folder chat 显式候选范围；文件夹递归展开，标签在已选范围内取交集，最终结果复用 F029 可见性过滤；无领域对象、表、API 路径、错误码或不变量变化 | F044 |
