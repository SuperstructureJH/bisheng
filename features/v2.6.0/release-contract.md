...
| UserPreference（`user_preference` 表） | F042-personal-font-size | 当前账号的电脑端个人显示偏好；本期仅拥有 `font_scale_level` 的读取与写入。中粮专版新值只接受 1 / 3 / 5，对应 90% / 100% / 110%；历史 1–7 值读取时归并。该对象按全局用户账号归属，不随租户切换，不修改源文档、导出内容或其他用户的显示 |
...
| F042-personal-font-size | — | 独立个人显示偏好；新增 `user_preference` 表与 `PUT /api/v1/user/preferences/font-size`，并在既有 `/api/v1/user/info` 返回当前档位；不依赖其他 v2.6.0 Feature，不新增错误码或跨 Feature 不变量 |
...
| 日期 | 变更内容 | 影响范围 |
|------|---------|---------|
| 2026-07-27 | F042 范围收敛：BISHENG 标品本期不发布；中粮专版电脑端固定为 1 / 3 / 5 三档，对应整页 90% / 100% / 110%，历史 1–7 档读取时归并；补充应用视口、Portal 与固定层不得留白或越界的边界合同 | F042 |
...
| 2026-07-24 | 登记 F042 个人字体大小的初始方案：新增账号级 `UserPreference` 领域对象与 `user_preference` 表；当时规划标品 7 档与中粮 1/3/5 三档，后由 2026-07-27 范围收敛记录替代；不新增错误码、不修改源内容与导出内容、不新增跨 Feature 不变量 | F042 |
